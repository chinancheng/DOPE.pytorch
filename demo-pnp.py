...
def draw(img, _vertexes, color=(0, 0, 255), scale=1):
    ...
    for p1, p2 in zip(line_1, line_2):
        if vertexes[p1] and vertexes[p2]:
            ...
            img = cv2.line(img, _p1, _p2, color=color, thickness=1)
...
model.load_state_dict(torch.load('./logs/logs-dr-pr-jock-2/checkpoint-best.pth'))
model.eval()
# path_to_data = '/media/ssd_external/DR-V3'
# path_to_data = './data/JockDM'
# path_to_data = './data/real'
# path_to_data = './data/JPEGImages'
path_to_data = './data/80'
# f = open(os.path.join(path_to_data, 'test.txt'))


# parameter of object and camera for pnp solver
object_size = json.load(open(os.path.join(path_to_data, '_object_settings.json')))['exported_objects'][0]['cuboid_dimensions']
# object_size = json.load(open(os.path.join(path_to_data, '_object_settings.json')))['exported_objects'][5]['cuboid_dimensions']
...
# matrix_camera[2,2] = 1
matrix_camera[0,0] = 941.45
matrix_camera[1,1] = 971.15
matrix_camera[0,2] = 637.62
matrix_camera[1,2] = 348.42
matrix_camera[2,2] = 1
...
path_to_sequences = [os.path.join(path_to_data, 'frame{}.jpg' .format(i)) for i in range(len(os.listdir(path_to_data)))]
id = 0
for path in path_to_sequences:
    ...
    if len(objects) > 0:
        for object in objects:
            ...
            draw(original_img, vertexes, scale=Config.stride*ratio)
            ...
            if valid_point_count >= 6:
                ...
                if ret:
                    ...
                    x, y, z = location
                    original_img = cv2.putText(original_img, 'pred_depth: ' + str(z//10), (50,150), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
                    if z < 0:
        ...
        # print('save output/pnp/img_{}.png' .format(id))
        original_img = cv2.putText(original_img, 'gt_depth: ' + str(80), (600, 150), cv2.FONT_HERSHEY_COMPLEX, 1,
                                   (0, 0, 255), 2)
        cv2.imwrite('output/depth_80/img_{}.png' .format(id), original_img)
        id += 1
