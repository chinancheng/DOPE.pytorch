...
def draw(img, vertexes, color=(0, 0, 255), scale=1):
    ...
    for p1, p2 in zip(line_1, line_2):
        if vertexes[p1] and vertexes[p2]:
            p1 = tuple([int(p * scale) for p in vertexes[p1]])
            p2 = tuple([int(p * scale) for p in vertexes[p2]])
...
model.load_state_dict(torch.load('./logs/logs-drv3-2/checkpoint.pth'))
model.eval()
...
for path in path_to_sequences:
    ...
    if len(objects) > 0:
        for object in objects:
            ...
            draw(original_img, vertexes, (255, 255, 0), scale=Config.stride*ratio)
